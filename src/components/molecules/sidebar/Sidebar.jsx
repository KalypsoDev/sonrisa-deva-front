...
return (
    <>
        ...
        <aside
            id="logo-sidebar"
            className="fixed top-0 z-40 w-56 h-screen transition-transform -translate-x-full sm:translate-x-0"
            aria-label="Sidebar"
        >
            <div className="h-full px-2 py-3 overflow-y-auto bg-primaryLila">
                ...
                <Link to="/admin" className="flex justify-center mb-6">
                    <img
                        className="rounded-full w-24 h-24 border-2 border-primaryBlue"
                        src={sonrisaDevaSmallLogo}
                        alt="Sonrisa de Deva Logo"
                    />
                </Link>
                ...
                <div className="flex justify-center">
                    <button onClick={handleLogout} className="text-white font-montserratBold text-sm bg-darkBlue focus:ring-4 focus:ring-blue-300 rounded-lg px-5 py-2.5 me-2 mb-2 mt-2 focus:outline-none">Cerrar sesión</button>
                </div>
            </div>
        </aside>
    </>
